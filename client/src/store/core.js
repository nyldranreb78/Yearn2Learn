  ...
  actions: {
      ...
      try {
        ...
        if (response) {
          ...
          for (const folder of this.folderList) {
            folder.notes = await Promise.all(
    ...
    async editFolder(folderId, folderName, folderPriority) {
      try {
        ...
        const folderIndex = this.folderList.findIndex(
          (folder) => folder._id === folderId,
        );

        if (folderIndex !== -1) {
          this.folderList[folderIndex] = {
            ...this.folderList[folderIndex],
            ...updatedFolder,
          };
        }
    ...
    async addNote(parentFolderId, noteTitle) {
      ...
      const folder = this.folderList.find(
      ...
      try {
        const savedNote = await this.authStore.createNoteInFolder(
          folder._id,
          note,
        );
    ...
    async addFlashcard(flashcardData) {
      try {
        const newFlashcard =
          await this.authStore.createFlashcard(flashcardData);
    ...
    async editFlashcard(flashcardId, flashcardData) {
      try {
        ...
        const flashcardIndex = this.flashcardList.findIndex(
          (flashcard) => flashcard._id === flashcardId,
        );

        if (flashcardIndex !== -1) {
          this.flashcardList[flashcardIndex] = {
            ...this.flashcardList[flashcardIndex],
            ...flashcardData,
          };
        }
    ...
    async deleteFlashcard(flashcardId) {
      try {
        ...
        this.flashcardList = this.flashcardList.filter(
          (flashcard) => flashcard._id !== flashcardId,
        );
      } catch (error) {
